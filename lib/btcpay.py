...
def validate (db, source, order_match_id, block_index):
    ...
    if order_match['backward_asset'] == config.BTC:
        if source != order_match['tx1_address'] and not (block_index >= 52000 or config.testnet):  # Protocol change.
        ...
        if source != order_match['tx0_address'] and not (block_index >= 52000 or config.testnet):  # Protocol change.
            problems.append('incorrect source address')
