...
def validate (db, source, quantity_per_unit, asset, dividend_asset, block_index):
    ...
    if asset == config.XCP:
        if (not block_index >= 125000) or block_index >= 193000 or config.TESTNET:   # Protocol change.
    ...
    if block_index >= 193000 or config.TESTNET:   # Protocol change.
    ...
    if not problems and dividend_asset != config.BTC:
        ...
        if block_index >= 420000 or config.TESTNET: # Protocol change.
...
def parse (db, tx, message):
    ...
    if status == 'valid':
        ...
        if tx['block_index'] >= 420000 or config.TESTNET: # Protocol change.
