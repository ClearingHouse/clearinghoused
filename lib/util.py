...
def price (numerator, denominator, block_index):
    if block_index >= 51100 or config.TESTNET: # Protocol change.
        ...
        if category == 'credits':
            ...
            if bindings['transfer']:
                ...
                if bindings['divisible']:
                    ...
                    unit = 1
                if bindings['callable'] and (bindings['block_index'] > 50100 or config.TESTNET):   # Protocol change.
                    callability = 'callable from {} for {} XCH/{}'.format(isodt(bindings['call_date']), bindings['call_price'], bindings['asset'])
                else:
            ...
            log_message = 'RPS: {} opens game with {} possible moves and a wager of {}'.format(bindings['source'], bindings['possible_moves'], output(bindings['wager'], 'XCH'))
            logging.info(log_message)

        elif category == 'rps_matches':
            log_message = 'RPS Match: {} is playing a {}-moves game with {} with a wager of {} ({}) [{}]'.format(bindings['tx0_address'], bindings['possible_moves'], bindings['tx1_address'], output(bindings['wager'], 'XCH'), bindings['id'], bindings['status'])
            logging.info(log_message)
            ...
            if bindings['status'] == 'valid':
                ...
                log_message = 'RPS Resolved: {} is playing {} on a {}-moves game with {} with a wager of {} ({}) [{}]'.format(rps_match['tx0_address'], bindings['move'], rps_match['possible_moves'], rps_match['tx1_address'], output(rps_match['wager'], 'XCH'), rps_match['id'], rps_match['status'])
            else:
...
def version_check (block_index):
    try:
        host = 'https://clearinghouse.github.io/clearinghoused/version.json'
    ...
    if not passed:
        explanation = 'Your version of clearinghoused is v{}, but, as of block {}, the minimum version is v{}.{}.{}. Reason: ‘{}’. Please upgrade to the latest version and restart the server.'.format(
...
def backend_check (db):
    ...
    if time_behind > 60 * 60 * 2:   # Two hours.
        raise BitcoindError('Viacoind is running about {} seconds behind.'.format(round(time_behind)))
...
def debit (db, block_index, address, asset, quantity, action=None, event=None):
    ...
    if protocol_change(block_index, 490000): # Protocol change.
        if len(address) == 40:
...
def asset_names_v2_enabled(block_index):
    return protocol_change(block_index, 490000)
