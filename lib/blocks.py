...
TABLES = ['credits', 'debits', 'messages'] + \
         ...
         'documents', 'document_transactions',
...
def get_tx_info1 (tx_hex, block_index, block_parser = None):
    ...
    for vout in ctx.vout:
        ...
        if len(asm) == 2 and asm[0] == 'OP_RETURN':                                             # OP_RETURN
            ...
            data_chunk = asm[1]
            #CH.TODO CHECK Viacoin specific: some encrypted tx leaked to mainnet prematurely, handle them along with plaintext.
                ...
                # If already skipped, skip it again.
                    ...
                    try:
                        with db:
                            ...
                            # List transaction.
                            try:    # Sometimes the transactions can’t be found: `{'code': -5, 'message': 'No information available about transaction'} Is txindex enabled in Viacoind?`
