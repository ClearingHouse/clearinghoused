...
def set_options (data_dir=None, backend_rpc_connect=None,
    ...
    if config.TESTCOIN:
        ...
        config.PREFIX = b'XCLRNGHS'             # 8 bytes
...
if __name__ == '__main__':
    ...
    parser.add_argument('--backend-rpc-connect', help='the hostname or IP of the backend viacoind JSON-RPC server')
    ...
    parser_burn = subparsers.add_parser('burn', help='destroy {} to earn XCH, during an initial period of time')
    ...
    parser_rpsresolve.add_argument('--fee', help='the exact VIA fee to be paid to miners')
    ...
    if args.action == 'send':
        ...
        if args.fee: args.fee = util.devise(db, args.fee, 'VIA', 'input')
        value = util.devise(db, args.value, 'XCH', 'input')
        startgas = util.devise(db, args.startgas, 'XCH', 'input')
